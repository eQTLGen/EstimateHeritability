...
h2_table_concat_annot <- h2_table_concat %>% inner_join(gene_annotations, by = c("gene_id"="ensembl_gene_id")) %>% filter(!marker_gene)

h2_table_pivot <- h2_table_concat_annot %>% filter(!marker_gene) %>% dplyr::select(gene_id, h2_obs, h2_obs_se, h2_z, h2_p, annot, lof.pLI, median_exp) %>% pivot_wider(values_from = c("h2_obs", "h2_obs_se", "h2_z", "h2_p"), names_from = "annot")

p <- ggplot(h2_table_pivot, aes(h2_z_cis, h2_z_trans, color = lof.pLI)) +
  geom_point(shape=16) + geom_smooth(method="lm")

ggsave("scatter_h2_11-14_trans.pdf", p, width = 4, height = 4)

p <- ggplot(h2_table_concat_annot, aes(h2_obs, median_exp, color = lof.pLI)) +
        geom_point(shape=16) +
        facet_wrap(vars(annot), scales="free_y", ncol = 2)

ggsave("exp_h2_11-14_trans.pdf", p, width = 4, height = 4)
