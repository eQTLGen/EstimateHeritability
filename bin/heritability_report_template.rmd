...
latent_factors_cis <- fread("latent_factors_cis.txt") %>% mutate(annot="cis")
latent_factors_trans <- fread("latent_factors_trans.txt") %>% mutate(annot="trans")
...
filtered <- merged_factors %>% filter(lhs == "GeNonCtc")

my_new_table <- tibble(h2_table_concat) %>% inner_join(filtered, by=c("gene_id"="rhs", "marker_gene", "annot")) %>%
        left_join(summarised_cell_type_factors, by = c("gene_id"="rhs", "annot")) %>%
        left_join(summarised_sign_cell_type_factors, by = c("gene_id"="rhs", "annot")) %>%
        rename(sum_of_cell_types = sum_of_ct_explained_variance, gene_expression = variance_explained) %>%
        rowwise() %>%
        mutate(sum_of_variance_explained = sum(sum_of_cell_types, gene_expression, na.rm=T),
               ratio_variance_explained_ge = gene_expression / sum_of_variance_explained) %>%
        ungroup()
...
p <- ggplot(my_new_table, aes(h2_obs, ratio_variance_explained_ge, colour = marker_gene, alpha = -log10(p_value))) +
        geom_point() + facet_wrap(vars(annot), nrow=2)
ggsave("explained_variance_ratio_h2_11-28_trans.pdf", p, width = 4, height = 4)
