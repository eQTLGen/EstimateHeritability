...
h2_table_trans <- fread(input_path_trans) %>%
  mutate(marker_gene = gene_id %in% mr_genes$ID)
h2_gw_trans <- fread(input_path_gw) %>%
  mutate(marker_gene = gene_id %in% mr_genes$ID)


h2_table_concat <- bind_rows(list("cis" = h2_table_cis, "trans" = h2_table_trans, "gw" = h2_gw_trans), .id = "annot")
...
ggsave("distributions_h2_10-31_trans.pdf", p, width = 6, height = 6)
